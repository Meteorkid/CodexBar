...
/// Plan Utilization 历史记录和每周限额重置检测的逻辑处理类。
@MainActor
final class PlanUtilizationStore {
    ...
    func planUtilizationHistory(for provider: UsageProvider) -> [PlanUtilizationSeriesHistory] {
        var providerBuckets = store.planUtilizationHistory[provider] ?? PlanUtilizationHistoryBuckets()
        ...
        store.planUtilizationHistory[provider] = providerBuckets
        if providerBuckets != originalProviderBuckets {
            let snapshotToPersist = store.planUtilizationHistory
            Task {
                await store.planUtilizationPersistenceCoordinator.enqueue(snapshotToPersist)
            }
        }
    ...
    func shouldShowRefreshingMenuCard(for provider: UsageProvider) -> Bool {
        let isRefreshing = store.isRefreshing || store.refreshingProviders.contains(provider)
        return isRefreshing
            && store.snapshots[provider] == nil
            && store.error(for: provider) == nil
    }
        ...
        async
    {
        ...
        await MainActor.run {
            var providerBuckets = store.planUtilizationHistory[provider] ?? PlanUtilizationHistoryBuckets()
            let preferredAccount = account ?? store.settings.selectedTokenAccount(for: provider)
            let accountKey = self.resolvePlanUtilizationAccountKey(
                provider: provider,
                snapshot: snapshot,
            ...
            store.planUtilizationHistory[provider] = providerBuckets
            snapshotToPersist = store.planUtilizationHistory
        }
        ...
        samples: [PlanUtilizationSeriesSample])
    {
        ...
        if let existingState = store.weeklyLimitResetDetectorStates[detectorKey],
    ...
    {
        let account = preferredAccount ?? store.settings.selectedTokenAccount(for: provider)
        let accountKey = Self.planUtilizationAccountKey(provider: provider, account: account)
        if let accountKey {
            return accountKey
        }
        let resolvedSnapshot = snapshot ?? store.snapshots[provider]
    ...
    private func persistWeeklyLimitResetDetectorStates() {
        do {
            let data = try JSONEncoder().encode(store.weeklyLimitResetDetectorStates)
            store.settings.userDefaults.set(data, forKey: Self.weeklyLimitResetDetectorDefaultsKey)
        } catch {
            CodexBarLog.logger(LogCategories.confetti).error(
        ...
        providerBuckets: inout PlanUtilizationHistoryBuckets) -> String?
    {
        ...
        let resolvedAccount = preferredAccount ?? store.settings.selectedTokenAccount(for: provider)
    ...
    private func resolveCodexPlanUtilizationAccountKey(
        ...
        providerBuckets: inout PlanUtilizationHistoryBuckets) -> String?
    {
        let ownership = store.codexOwnershipContext(snapshot: snapshot, includeDashboardFallback: true)
