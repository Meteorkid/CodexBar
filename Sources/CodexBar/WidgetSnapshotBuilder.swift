...
final class WidgetSnapshotBuilder {
    ...
    func persistWidgetSnapshot(reason: String) {
        ...
        let previousTask = store.widgetSnapshotPersistTask
        store.widgetSnapshotPersistTask = Task { @MainActor in
            _ = await previousTask?.result

            if let override = store._test_widgetSnapshotSaveOverride {
    ...
    private func makeWidgetSnapshot() -> WidgetSnapshot {
        let enabledProviders = store.enabledProviders()
    ...
    private func makeWidgetEntry(for provider: UsageProvider) -> WidgetSnapshot.ProviderEntry? {
        ...
        let tokenSnapshot = store.tokenSnapshots[provider]
