...
public struct ZhipuUsageSnapshot: Sendable {
    public let isConnected: Bool
    public let modelCount: Int
    public let modelNames: [String]
    public let balanceInfo: ZhipuBalanceInfo?
    ...
    public init(
        ...
        modelCount: Int,
        modelNames: [String],
        balanceInfo: ZhipuBalanceInfo? = nil,
        updatedAt: Date)
    {
...
public struct ZhipuBalanceInfo: Sendable {
...
public enum ZhipuUsageError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .missingCredentials:
            "Missing Zhipu API key."
        case .missingCookie:
            "Missing Zhipu session cookie."
        case let .networkError(message):
            "Zhipu network error: \(message)"
        case let .apiError(message):
            "Zhipu API error: \(message)"
        case let .parseFailed(message):
            "Failed to parse Zhipu response: \(message)"
        }
    }
}
...
public struct ZhipuUsageFetcher: Sendable {
    private static let log = CodexBarLog.logger(LogCategories.zhipuUsage)
    private static let webLog = CodexBarLog.logger(LogCategories.zhipuWeb)
    private static let baseURL = URL(string: "https://open.bigmodel.cn/api/paas/v4")!
        ...
        baseURL: URL = URL(string: "https://open.bigmodel.cn/api/paas/v4")!) async throws -> ZhipuUsageSnapshot
    {
        do {
            ...
            case let .networkError(message):
                throw ZhipuUsageError.networkError(message)
            case let .apiError(message):
                throw ZhipuUsageError.apiError(message)
            case let .parseFailed(message):
                throw ZhipuUsageError.parseFailed(message)
            }
        }
    }

    public static func fetchBalance(cookieHeader: String, now: Date = Date()) async throws -> ZhipuUsageSnapshot {
        ...
        request.setValue("https://open.bigmodel.cn/usercenter", forHTTPHeaderField: "Referer")

        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse else {
            throw ZhipuUsageError.networkError("Invalid response")
        }

        guard httpResponse.statusCode == 200 else {
            let body = String(data: data, encoding: .utf8) ?? "<binary>"
            Self.webLog.error("Zhipu balance API returned \(httpResponse.statusCode): \(body)")
            if httpResponse.statusCode == 401 || httpResponse.statusCode == 403 {
                throw ZhipuUsageError.apiError("Cookie expired or invalid")
            }
            throw ZhipuUsageError.apiError("HTTP \(httpResponse.statusCode)")
        }
    ...
    static func _parseBalanceForTesting(_ data: Data) throws -> ZhipuBalanceInfo {
        try self.parseBalanceResponse(data: data)
    }

    private static func parseBalanceResponse(data: Data) throws -> ZhipuBalanceInfo {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw ZhipuUsageError.parseFailed("Invalid JSON")
        }
